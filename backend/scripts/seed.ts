...
import { DEPARTMENTS, EMPLOYMENT_TYPES } from "../src/constants/employee";
...
const parseArgFlag = (flag: string) => process.argv.slice(2).includes(flag);

const loadNames = (fileName: string) => {
...
const createSalary = (minSalary: number, maxSalary: number) => {
...
const run = async () => {
  ...
  const [{ count }] = await db("employees").count<{ count: string }[]>("id as count");
  ...
  for (let index = 0; index < TARGET_EMPLOYEE_COUNT; index += 1) {
    ...
    rows.push({
      ...
      employment_type: randomPick([...EMPLOYMENT_TYPES]),
